...
Search — tableSearch identified candidate tables matching visit_occurrence across Unity Catalog.
Certification check — readTable with extraDetails=true confirmed that main.omop.visit_occurrence carries a certified governance tag, making it the authoritative source.
Schema extraction — The 17-column schema (types, nullability, column comments) was read from the certified table and faithfully mapped to a PySpark StructType.
Metadata propagation — After writing the synthetic DataFrame to dmoore.project-2026-april.visit_occurrence, all metadata was propagated from the certified source in separate DDL statements: table comment, column comments, table-level tags (industry = HLS), and column-level tags (phi = true).
